...
4. **Auto-Rating System**: Teams that don't submit peer ratings receive an automatic rating of **6.5/10** (neutral midpoint of 3-10 scale). This is included in P_avg calculations.

5. **P_norm Fallback**: When no peer ratings exist for a team, P_norm defaults to **0.5** (neutral score) instead of 0 for fairness.

6. **Cascading Tiebreaker**: If teams have identical final scores, the following 6-level tiebreaker is applied:
...
7. **Idempotent Submissions**: Votes, peer ratings, and judge scores return existing records with 200 status when resubmitted. Quiz submissions are NOT idempotent (returns 409 on duplicate for educational integrity).
